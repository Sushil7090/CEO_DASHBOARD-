...
router.get("/financial-summary", async (req, res) => {
  try {
    ...
    const profit_margin =
      total_revenue > 0 ? ((profit / total_revenue) * 100).toFixed(2) : 0;

    const budget_utilization =
      total_budget > 0 ? ((total_cost / total_budget) * 100).toFixed(2) : 0;
...
router.get("/payment-status", async (req, res) => {
  try {
    ...
    invoices.forEach((i) => {
      const amount = Number(
        i.total_amount || i.invoice_amount || i.amount || 0,
      );
      ...
      if (
        ...
        const delay_days = Math.ceil((today - dueDate) / (1000 * 60 * 60 * 24));
      ...
      if (["Paid", "PAID", "paid"].includes(status)) {
        ...
        if (i.payment_date && dueDate) {
          const delay =
            (new Date(i.payment_date) - dueDate) / (1000 * 60 * 60 * 24);
...
router.get("/analytics", async (req, res) => {
  try {
    ...
    projects.forEach((p) => {
      ...
      budgetByCategory[category].total_spent += forceNumber(
        p.total_cost_to_date,
      );
    });
